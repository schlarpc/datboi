...
//!
//! 1. **Predicate** (never eager): of the rebuild's input bytes that
//!    would ever need packing, the fraction that is shared (claimed by
//!    ≥2 decompositions) or already resident must clear the molten
//!    threshold (`swap:share-min-pct`). A lone ROM's swap buys pad
//!    savings and pays an inode-and-IO bill, so it never trips.
//!    GENERATED inputs (a zero-input route — the D111 filler stream)
//!    are outside that fraction entirely: they cost nothing to hold and
//!    are never packed. They carry a second trigger instead: when
//!    generated bytes are at least `swap:generated-min-pct` of the
//!    CONTAINER, the swap fires regardless of sharing — a lone seed-era
//!    Xbox disc drops ~40% of its bytes for one pack write.
...
impl<'s> Executor<'s> {
    ...
    pub fn swap_covered(&self, db: &mut Db) -> Result<SwapReport, ExecError> {
        ...
        let generated_threshold = u64::from(policy::swap_generated_min_pct(db)?);
        for candidate in db.swap_candidates()? {
            match self.swap_one(db, &candidate, threshold, generated_threshold, &mut report) {
                Ok(()) => {}
    ...
    fn swap_one(
        ...
        for input in &inputs {
            ...
            // Generated inputs (D111) are never stored at all: outside
            // the sharing fraction, counted for their own trigger.
            if input.generated {
                if seen_generated.insert(input.hash) {
                    generated += size;
                }
                continue;
            }
        ...
        // or all-generated rebuilds) has nothing to pack. The D111
        // trigger measures generated bytes against the CONTAINER (the
        // reclaim is container bytes, not input bytes).
        let regenerates_enough = candidate
            .size
            .is_some_and(|c| c > 0 && generated * 100 >= c * generated_threshold_pct);
        if total == 0 || (shared * 100 < total * threshold_pct && !regenerates_enough) {
