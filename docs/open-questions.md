...
- **Container retention, the unruled half** (D123 ruled the rest).
  What is ruled: retention is a choice, not a format property; the
  default stays retain; `--unpack` / `datboi unpack` are the operator's
  two doors; a dropped container keeps its row, aliases, `source_file`
  provenance and its `container→member` recipes, and goes `Absent`.
  What stays open is the AUTOMATIC half, and it is the same shape D121
  left open for `--materialize`: should the daemon unpack on its own —
  driven by view pinning, dat-awareness, or a refine verdict that a
  container is provably unpreflatable (D53's coverage gap) — and under
  what watermark? That needs the residency policy this repo has not
  written yet, not a flag. Two inputs it will want: the preflate backlog
  rate (~2 members per 3 minutes against 141,986 on the live host, so
  "wait for refine" is not an answer on its own), and whether plaintext
  CDC — measurable for the first time once members are resident —
  changes the 1.13× cross-zip sharing number that made unpacking a
  +66 GB trade rather than a win.
