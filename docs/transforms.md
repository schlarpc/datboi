...
| CHD | read only | **not across chdman versions** | yes | no | reading is native and in-tree (`datboi-formats::chd`): v1–v5 headers and maps, codecs none/zlib/lzma/huff/flac/cdzl/cdlz/cdfl, refusing `avhu` and delta CHDs by name. The `chd-verify` family decompresses a CHD and grades the match have-verified (D44 amendment). No decomposition and no writer: D123 rules hunk-as-blob out (FastCDC already generalises it), and a byte-exact rebuild needs a corrections lane per codec |
