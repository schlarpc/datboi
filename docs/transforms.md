...
| CHD | read only | **not across chdman versions** | yes | no | reading is native and in-tree (`datboi-formats::chd`): v1–v5 headers and maps, codecs none/zlib/lzma/huff/flac/cdzl/cdlz/cdfl, refusing `avhu` and delta CHDs by name. The `chd-verify` family decompresses a CHD and grades the match have-verified (D44 amendment). No decomposition and no writer: D124 rules hunk-as-blob out (FastCDC already generalises it), and a byte-exact rebuild needs a corrections lane per codec |
...
| Wii decomposition + partition re-encrypt | in | yes | yes (2 MiB groups) | no | native walker + `xf-wii-crypt` (D116): disc structures and partition bodies as pieces, each verified body's plaintext split by the GameCube walker (files + junk regen, seeded from the partition's own ID), rebuilt by `encrypt {wrapped title key, title id, sectors}` over the plaintext and the common key (found by known hash, D12; a missing key defers the sweep item); the swap packs the grounding leaves only; Brawl: 15 partitions verified, 9.0% regenerated
